...
export function JobsScreen() {
  ...
  const loadJobs = useCallback(
    async (mode: 'initial' | 'refresh') => {
      ...
      if (!cacheOwner) {
        setErrorMessage('Could not identify the current field agent session.');
      ...
      try {
        ...
        try {
          await saveCachedAssignedJobs(
            cacheOwner,
            response.data,
            nextLastSyncedAt,
          );
        } catch {
          // Live data should remain usable even if local cache persistence fails.
        }
      } catch (error) {
        if (canFallbackToAssignedJobsCache(error)) {
          ...
          if (cachedJobs) {
            ...
            setLastSyncedAt(cachedJobs.lastSyncedAt);
          ...
          setIsShowingCachedData(false);
          setErrorMessage(
            'Live data is unavailable, and no saved jobs are stored on this device yet. Connect once to sync assigned jobs.',
          );
          return;
        }
    ...
    [authenticatedRequest, cacheOwner],
  );
    ...
    <View style={styles.header}>
      ...
      {isShowingCachedData ? (
        <View style={styles.savedDataBanner}>
          <Text style={styles.savedDataTitle}>Showing saved jobs</Text>
          <Text style={styles.savedDataText}>
            Live data is unavailable. {formatLastSyncedAt(lastSyncedAt)}.
          </Text>
        </View>
      ) : lastSyncedAt ? (
